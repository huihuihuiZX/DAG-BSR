from option.DAGBSR.option_train import args
from data.srdataset import SRDataset
from torch.utils.data import DataLoader
from utils import degradation1, utility
...
def train(args, train_loader, model, criterion_sr, criterion_dp, optimizer, epoch, log_file_path):
    ...
    if args.blur_type_prob == [1,0]:
        degrade = degradation1.SimpleDegradation(args)
    else:
        degrade = degradation1.MildDegradation(args)
...
def test(args, test_loader, model, log_file_path):
    ...
    with torch.no_grad():
        ...
        for batch, (hr, lr, _) in enumerate(test_loader):
            ...
            hr = crop_border_win(hr, args.scale, args.window_size)
            lr = crop_border_win(lr, 1, args.window_size)
