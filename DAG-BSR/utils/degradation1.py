...
def generate_stable_kernel(kernel_size=21, blur_type='iso_gaussian', sigma=1.8, theta=0, sigma_x=0.2, sigma_y=4.0):
    if blur_type == 'iso_gaussian':
        ...
        return stable_anisotropic_gaussian_kernel(kernel_size=kernel_size, theta=theta, sigma_x=sigma_x,
                                                  sigma_y=sigma_y)
...
class BatchNoise(nn.Module):
    ...
    def __call__(self, lr_downsampled):
        ...
        noise_level = torch.rand(int(B / 2), 1, 1, 1, 1).cuda()* self.noise_max
        noise = torch.randn_like(lr_downsampled).reshape(-1, 2, C, H, W).mul_(noise_level).view(-1, C, H, W)
...
class StableJpeg(nn.Module):
    def __init__(self, mode=255):
        super(StableJpeg, self).__init__()
        self.mode = mode

    def __call__(self, lr_noised, jpeg):
        B, C, W, H = lr_noised.shape
        if self.mode == 255:
            lr_noised = lr_noised / 255.0
        lr_jpeged = torch.zeros(B, C, W, H).float().cuda()
...
class SimpleDegradation(object):
    ...
    def __call__(self, hr_tensor):
        with torch.no_grad():
            ...
            b_kernels = self.gen_kernel(B)
            hr_tensor = hr_tensor.reshape(B, -1, H, W)
...
class MildDegradation(object):
    def __init__(self, args):
        ...
        self.noise = BatchNoise(args.noise_min, args.noise_max)
...
class StableMildDegradation(object):
    def __init__(self, args):
        self.scale = args.scale
        self.kernel_size = args.kernel_size

        self.gen_kernel = BatchStableKernel(args.blur_type_list, args.blur_type_prob, args.kernel_size, args.sigma,
                                            args.theta, args.sigma_min, args.sigma_max)
        self.blur = BatchBlur(args.kernel_size)

        self.down_sample = BatchBicubic(args.scale)

        self.noise = StableNoise(args.noise)
...
class AnisoDegradation(object):
    def __init__(self, theta, sigma_min, sigma_max, noise):
        self.scale = 4
        kernel_size = 21
        blur_type_list = ['iso_gaussian', 'aniso_gaussian']
        blur_type_prob = [0, 1]
        self.gen_kernel = BatchStableKernel(blur_type_list, blur_type_prob, kernel_size, 0.0, theta, sigma_min, sigma_max)
        self.blur = BatchBlur(kernel_size)
        self.down_sample = BatchBicubic(self.scale)
        self.noise = StableNoise(noise)
